...
//cheungbx - display instructions to select button/paddle/demo
   ...
   arduboy.print("Breakout on Paddle");
   arduboy.setCursor(20, 15);
   arduboy.print("A = Sound On");   
   arduboy.setCursor(20, 30);
   arduboy.print("B = Sound Off");
   arduboy.setCursor(10, 45);
   arduboy.print("Paddle");
   arduboy.setCursor(60, 45);
   arduboy.print(minPaddle);
      arduboy.setCursor(90, 45);
   arduboy.print(maxPaddle);
...
// Adjust minPaddle and maxpaddle
    paddleV = analogRead(paddle1X);
    if (paddleV > maxPaddle) 
      maxPaddle=paddleV-10;
    else if (paddleV < minPaddle) 
      minPaddle=paddleV+10;
...
if (usePaddle) {
  if (usePaddle)
    racket = map (constrain (analogRead(paddle1X), minPaddle, maxPaddle), minPaddle, maxPaddle, 0, SCRNWIDTH-RACKETSIZE);
}
else {  
